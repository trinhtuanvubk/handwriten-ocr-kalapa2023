...
def augmentation(image):
    ...
    A.ShiftScaleRotate( shift_limit = 0.0, shift_limit_y = 0.02, scale_limit=0.0, rotate_limit=1, p=0.2),
    ...
    for _ in range(10):
        ...
        # Define the desired height and width for the cropped image
            ...
            desired_width = int(image.shape[1] * random.uniform(0.97,1))  # Crop random % from the right
...
def process_image(root, image_path, name, annotation):
    input_path = os.path.join(root, "images")
    anno_input_path = os.path.join(root, "labels")
    image_aug_list = process_line(image_path)
